...
while(<C>)
{
	...
	next if ($mutation_somatic_status ne "Confirmed somatic variant" and $sample_source ne "cell-line");
	$gene_name =~ s/_ENST.*//;
...
while (my $line = $vcf->next_line())
{
	...
	my ($dp, $freq, $ad_ref, $ad_alt, $ref_fwd, $ref_rev, $var_fwd, $var_rev, $pval);
	
	##FORMAT=<ID=RD,Number=1,Type=Integer,Description="Depth of reference-supporting bases (reads1)">
	##FORMAT=<ID=AD,Number=1,Type=Integer,Description="Depth of variant-supporting bases (reads2)">
	$ad_ref = $x->{gtypes}{$patient_sample}{RD};
	$ad_alt = $x->{gtypes}{$patient_sample}{AD};

	##FORMAT=<ID=DP,Number=1,Type=Integer,Description="Read Depth">				
	$dp = $x->{gtypes}{$patient_sample}{DP};

	##FORMAT=<ID=FREQ,Number=1,Type=String,Description="Variant allele frequency">
	$freq = $x->{gtypes}{$patient_sample}{FREQ};
	$freq =~ s/\%//;
	...
	# ----- annotate ClinVar variants
	{
		...
		if ($iter and $iter->{_})
		{
			while (my $line = $clinvar->read($iter)) 
			{
				...
				if ($pos eq $rpos and $x->{REF} eq $ref_allele and $x->{ALT}->[0] eq $alt_allele)
				{
	...
	print $max_g1k_freq > 0 ? sprintf("%.4f", $max_g1k_freq) : "", "\t";	
	print $max_evs_freq > 0 ? sprintf("%.4f", $max_evs_freq) : "", "\t";
